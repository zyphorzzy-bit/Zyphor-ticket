...
const padrao = {
  tipoAbertura: 'botoes', corBotao: 'Primary', criarComo: 'canal', categoriaPadrao: '',
  cargos: { c1: '', c2: '', c3: '' },
  opcoes: [
    { label: 'Suporte', emoji: '❓', value: 'suporte' },
    { label: 'Denúncia', emoji: '🚨', value: 'denuncia' },
    { label: 'Parceria', emoji: '🤝', value: 'parceria' },
    { label: 'Sorteio', emoji: '🎁', value: 'sorteio' }
  ]
};
...
const painel = () => ({
  embeds: [new EmbedBuilder().setColor(0x2b2d31).setTitle(`${E.config} CENTRAL DE CONFIGURAÇÃO`).addFields(
    { name: `${E.id} Tipo`, value: config.criarComo === 'canal' ? '📂 Canal' : '🧵 Tópico', inline: true },
    { name: `${E.alerta} Modos`, value: config.tipoAbertura === 'selectmenu' ? `${E.ativado} Menu` : `${E.desativado} Botões`, inline: true },
    { name: `${E.edit} Categoria ID`, value: config.categoriaPadrao ? `\`${config.categoriaPadrao}\`` : '❌ Não definida', inline: true },
    { name: `${E.user} Cargos`, value: Object.values(config.cargos).filter(Boolean).map(c => `<@&${c}>`).join(', ') || 'Nenhum', inline: false },
    { name: `${E.linkexterno} Opções`, value: config.opcoes.map((o, i) => `**${i + 1}.** ${o.emoji} ${o.label}`).join('\n') || 'Nenhuma', inline: false }
  )],
  components: [
...
client.on(Events.InteractionCreate, async int => {
    ...
    if (int.commandName === 'enviar') {
      ...
      if (config.tipoAbertura === 'selectmenu') {
        ...
        config.opcoes.forEach((op, i) => {
          ...
          row.addComponents(new ButtonBuilder().setCustomId(`btn_${op.value}`).setLabel(op.label).setEmoji(op.emoji).setStyle(ButtonStyle[config.corBotao] || ButtonStyle.Primary));
        });
  ...
  if (int.isButton() && int.customId.startsWith('cfg_')) {
    ...
    if (int.customId === 'cfg_cat') return int.reply({ content: `${E.alerta} Use no chat: \`!setcat ID_DA_CATEGORIA\``, ephemeral: true });
  ...
  if ((int.isStringSelectMenu() && int.customId === 'menu_abrir') || (int.isButton() && int.customId.startsWith('btn_'))) {
    ...
    try {
      if (config.criarComo === 'canal') {
        ...
        const channelOpts = { name, type: ChannelType.GuildText, permissionOverwrites };
        const catClean = (config.categoriaPadrao || '').replace(/\D/g, '');
        if (catClean) channelOpts.parent = catClean;

        canal = await int.guild.channels.create(channelOpts);
      } else {
      ...
      return int.reply({ content: `${E.proibido} Erro ao criar o ticket! Verifique se o ID da Categoria em \`!setcat\` está correto ou limpe com \`!setcat reset\`.`, ephemeral: true });
    }
  }
...
client.on(Events.MessageCreate, async m => {
  ...
  if (cmd === '!setcat') {
    const catId = args[0];
    if (catId === 'reset') { config.categoriaPadrao = ''; salvarCfg(); return m.reply(`${E.ativado} Categoria removida!`); }
    config.categoriaPadrao = catId || m.channel.parentId || '';
    salvarCfg();
    return m.reply(`${E.ativado} Categoria padrão salva: \`${config.categoriaPadrao}\``);
  }
});
