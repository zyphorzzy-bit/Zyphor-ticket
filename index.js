...
client.on(Events.InteractionCreate, async int => {
  ...
  if (int.isButton() && int.customId.startsWith('cfg_')) {
    ...
    if (int.customId === 'cfg_cat_cmd') return int.reply({ content: `${E.alerta} Use no chat: \`!setcat ID_DA_CATEGORIA\` (ou só \`!setcat\` dentro do canal da categoria desejada).`, ephemeral: true });
    ...
    if (int.customId === 'cfg_midia_modal') {
      const modal = new ModalBuilder().setCustomId('modal_midia').setTitle('Banner e Thumbnail (Links de Fotos)');
      modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('banner').setLabel('URL do Banner (Grande abaixo)').setStyle(TextInputStyle.Short).setValue(config.embedImage || '').setRequired(false)),
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('thumb').setLabel('URL da Thumbnail (Miniatura do lado)').setStyle(TextInputStyle.Short).setValue(config.embedThumb || '').setRequired(false))
      );
      return int.showModal(modal);
    }

    if (int.customId === 'cfg_add_modal') {
      const modal = new ModalBuilder().setCustomId('modal_add').setTitle('Adicionar Categoria/Opção');
      modal.addComponents(
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('emoji').setLabel('Emoji (Comum ou Customizado)').setStyle(TextInputStyle.Short).setPlaceholder('Ex: ❓ ou <:nome:123456789>').setRequired(true))
      );
      return int.showModal(modal);
    }
    ...
    if (int.customId === 'cfg_rem_modal') {
      ...
      modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('num').setLabel('Número da Opção que deseja apagar (1, 2...)').setStyle(TextInputStyle.Short).setRequired(true))
      );
      return int.showModal(modal);
    }
    ...
    if (int.customId === 'cfg_edit_modal') {
      ...
      modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('num').setLabel('Número da Opção (1, 2, 3...)').setStyle(TextInputStyle.Short).setRequired(true)),
  ...
  if ((int.isStringSelectMenu() && int.customId === 'menu_abrir') || (int.isButton() && int.customId.startsWith('btn_'))) {
    ...
    return int.reply({ content: `${E.ativado} Ticket criado com sucesso: ${canal}`, ephemeral: true });
  }

  // Interações internas do Ticket
  if (int.isButton()) {
    if (int.customId === 'atender_ticket') {
      if (!eStaff(int.member)) return int.reply({ content: `${E.proibido} Apenas a equipe pode assumir!`, ephemeral: true });
...
client.on(Events.MessageCreate, async m => {
  ...
  if (['.f', '.fechar'].includes(cmd) && t) {
    ...
    await m.reply(`${E.fechar} Encerrando...`);
  ...
  if (cmd === '.adc' && eStaff(m.member) && t) {
    ...
    else await m.channel.permissionOverwrites.edit(alvo.id, { ViewChannel: true, SendMessages: true, AttachFiles: true });
