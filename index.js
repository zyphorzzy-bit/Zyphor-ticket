...
client.on(Events.InteractionCreate, async int => {
  ...
  if ((int.isStringSelectMenu() && int.customId === 'menu_abrir') || (int.isButton() && int.customId.startsWith('btn_'))) {
    ...
    if (catId && catId.startsWith('<#') && catId.endsWith('>')) {
      catId = catId.slice(2, -1);
    }
    ...
    try {
      ...
      console.error(err);
      return int.reply({ content: `${E.proibido} Erro ao criar o ticket! Verifique as permissões do bot no servidor e se o ID da Categoria em \`!setcat\` está correto.`, ephemeral: true });
    }
  }
...
client.on(Events.MessageCreate, async m => {
  ...
  if (m.content.startsWith('!setcat')) {
    const [, cat, catId] = m.content.split(' ');
    ...
    salvarCfg();
    return m.reply(`${E.ativado} Categoria \`${cat}\` configurada com sucesso!`);
  }
