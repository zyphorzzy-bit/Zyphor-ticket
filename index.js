...
// =========================================================
// 🎨 EMOJIS PERSONALIZADOS DO BOT (PODE ALTERAR AQUI):
// =========================================================
const E = {
  ...
  fechar: '<:fechar:1539890262690758666>',
  atender: '<:atender:1539890260761382944>'
};
...
client.on(Events.InteractionCreate, async int => {
  ...
  if (int.isButton() && int.customId.startsWith('cfg_')) {
    ...
    // Modal para Adicionar Opção / Emoji Personalizado
    if (int.customId === 'cfg_add_modal') {
      const modal = new ModalBuilder().setCustomId('modal_add').setTitle('Nova Opção');
      modal.addComponents(
        ...
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('emoji').setLabel('Emoji (Comum ou Personalizado)').setStyle(TextInputStyle.Short).setPlaceholder('Ex: ❓ ou <:nome:123456789>').setRequired(true))
      );
      return int.showModal(modal);
    }

    // Modal para Editar Opção / Emoji Existente
