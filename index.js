...
const padrao = {
  tipoAbertura: 'botoes', corBotao: 'Primary', criarComo: 'canal', catId: '', logChannelId: '',
  embedTitulo: 'CENTRAL DE ATENDIMENTO', embedDesc: 'Selecione uma opção abaixo para abrir seu ticket.',
  embedImage: '', embedThumb: '', statusTexto: 'Atendimento On-line',
  cargos: { c1: '', c2: '', c3: '' },
  opcoes: [
...
// Checagem de permissão Staff/Dono
...
const painel = () => {
  ...
  return {
    embeds: [new EmbedBuilder().setColor(0x2b2d31).setTitle(`${E.config} PAINEL DE CONFIGURAÇÃO`).addFields(
      ...
      { name: `${E.edit} Banner/Foto`, value: config.embedImage ? `[Banner Setado](${config.embedImage})` : 'Nenhum', inline: true },
      { name: `${E.edit} Thumbnail`, value: config.embedThumb ? `[Miniatura Setada](${config.embedThumb})` : 'Nenhuma', inline: true },
      { name: `${E.linkexterno} Opções do Ticket`, value: opcoesTxt, inline: false }
    )],
    components: [
      new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder().setCustomId('cfg_add_modal').setLabel('+ Opção').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId('cfg_rem_modal').setLabel('- Remover').setStyle(ButtonStyle.Danger),
        new ButtonBuilder().setCustomId('cfg_edit_modal').setLabel('Editar Opção').setStyle(ButtonStyle.Secondary)
      ),
      new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId('cfg_embed_modal').setLabel('Textos').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('cfg_midia_modal').setLabel('Fotos/Banner').setStyle(ButtonStyle.Primary),
        new ButtonBuilder().setCustomId('cfg_cat_cmd').setLabel('Setar Categoria').setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('cfg_logs_cmd').setLabel('Setar Logs').setStyle(ButtonStyle.Secondary),
...
client.on('ready', async () => {
  console.log(`${E.ativado} Bot Online como: ${client.user.tag}`);
  client.user.setPresence({
    activities: [{ name: 'Suporte Zyphor', type: ActivityType.Streaming, url: 'https://twitch.tv/discord' }],
    status: 'online'
  });
...
client.on(Events.InteractionCreate, async int => {
  ...
  if (int.isButton() && int.customId.startsWith('cfg_')) {
    ...
    if (int.customId === 'cfg_cat_cmd') return int.reply({ content: `${E.alerta} Use no chat: \`!setcat ID_DA_CATEGORIA\``, ephemeral: true });
    if (int.customId === 'cfg_logs_cmd') return int.reply({ content: `${E.alerta} Use no chat: \`!setlogs #canal-de-logs\``, ephemeral: true });
    ...
    if (int.customId === 'cfg_midia_modal') {
      const modal = new ModalBuilder().setCustomId('modal_midia').setTitle('Banner e Thumbnail');
      modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('banner').setLabel('URL do Banner').setStyle(TextInputStyle.Short).setValue(config.embedImage || '').setRequired(false)),
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('thumb').setLabel('URL da Thumbnail').setStyle(TextInputStyle.Short).setValue(config.embedThumb || '').setRequired(false))
      );
      return int.showModal(modal);
    }

    if (int.customId === 'cfg_add_modal') {
      const modal = new ModalBuilder().setCustomId('modal_add').setTitle('Adicionar Opção');
      modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('nome').setLabel('Nome da Opção').setStyle(TextInputStyle.Short).setRequired(true)),
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('emoji').setLabel('Emoji').setStyle(TextInputStyle.Short).setRequired(true))
      );
      return int.showModal(modal);
    }

    if (int.customId === 'cfg_rem_modal') {
      const modal = new ModalBuilder().setCustomId('modal_rem').setTitle('Remover Opção');
      modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('num').setLabel('Número da Opção').setStyle(TextInputStyle.Short).setRequired(true))
      );
      return int.showModal(modal);
    }

    if (int.customId === 'cfg_edit_modal') {
      const modal = new ModalBuilder().setCustomId('modal_edit').setTitle('Editar Opção');
      modal.addComponents(
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('num').setLabel('Número da Opção').setStyle(TextInputStyle.Short).setRequired(true)),
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('nome').setLabel('Novo Nome').setStyle(TextInputStyle.Short).setRequired(true)),
        new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('emoji').setLabel('Novo Emoji').setStyle(TextInputStyle.Short).setRequired(true))
      );
      return int.showModal(modal);
    }
  ...
  // RESPOSTAS DOS MODAIS DE CONFIGURACAO
  if (int.isModalSubmit()) {
    if (int.customId === 'modal_embed') {
      config.embedTitulo = int.fields.getTextInputValue('t');
      config.embedDesc = int.fields.getTextInputValue('d');
    }
    if (int.customId === 'modal_midia') {
      config.embedImage = int.fields.getTextInputValue('banner');
      config.embedThumb = int.fields.getTextInputValue('thumb');
    }
    if (int.customId === 'modal_add') {
      config.opcoes.push({ label: int.fields.getTextInputValue('nome'), emoji: int.fields.getTextInputValue('emoji'), value: `op_${Date.now().toString().slice(-4)}` });
    }
    if (int.customId === 'modal_rem') {
      const idx = parseInt(int.fields.getTextInputValue('num')) - 1;
      if (!isNaN(idx) && config.opcoes[idx]) config.opcoes.splice(idx, 1);
    }
    if (int.customId === 'modal_edit') {
      const num = parseInt(int.fields.getTextInputValue('num')) - 1;
      if (!isNaN(num) && config.opcoes[num]) {
        config.opcoes[num].label = int.fields.getTextInputValue('nome');
        config.opcoes[num].emoji = int.fields.getTextInputValue('emoji');
      }
    }
    salvarCfg(); return int.update(painel());
  }
  ...
  if ((int.isStringSelectMenu() && int.customId === 'menu_abrir') || (int.isButton() && int.customId.startsWith('btn_'))) {
    ...
    // Evita travamento no Select Menu
    ...
    if (db.userTickets[int.user.id]) {
      const msg = `${E.alerta} Você já possui um ticket aberto: <#${db.userTickets[int.user.id]}>`;
      return int.isStringSelectMenu() ? int.followUp({ content: msg, ephemeral: true }) : int.editReply({ content: msg });
    }
    ...
    if (config.criarComo === 'canal') {
      const permissionOverwrites = [
        ...
        { 
          ...
          deny: [PermissionsBitField.Flags.ManageChannels, PermissionsBitField.Flags.ManageRoles] 
        }
      ];
      ...
      canal = await int.channel.threads.create({ name, type: ChannelType.PrivateThread, autoArchiveDuration: 1440 });
      await canal.members.add(int.user.id);
    }
    ...
    await canal.send({ content: `<@${int.user.id}> ${pingsStaff}`, embeds: [emb], components: [btns] });
    
    const resposta = `${E.ativado} Ticket criado com sucesso: ${canal}`;
    return int.isStringSelectMenu() ? int.followUp({ content: resposta, ephemeral: true }) : int.editReply({ content: resposta });
  }

  // INTERAÇÕES NOS BOTOES DO TICKET
  if (int.isButton()) {
    // ASSUMIR TICKET
    if (int.customId === 'atender_ticket') {
      if (!eStaff(int.member)) {
        return int.reply({ content: `${E.proibido} Você não tem permissão para assumir este ticket!`, ephemeral: true });
      }

      // Remove o botão de assumir e deixa apenas o de fechar
    ...
    // FECHAR E DELETAR TICKET
    if (int.customId === 'fechar_ticket') {
      const t = db.tickets?.[int.channel.id];
      if (!t || (!eStaff(int.member) && t.dono !== int.user.id)) {
        return int.reply({ content: `${E.proibido} Apenas a equipe autorizada ou o dono podem fechar este ticket!`, ephemeral: true });
      }
      ...
      await int.reply(`${E.fechar} Encerrando e excluindo o ticket em instantes...`);

      setTimeout(async () => {
        try {
          await int.channel.delete();
        } catch {
          if (int.channel.isThread()) await int.channel.delete();
        }
      }, 2000);
    }
  }
...
// COMANDOS DE CHAT (.f, .adc, !setcat, !setlogs)
client.on(Events.MessageCreate, async m => {
  ...
  // Fechar ticket via comando
  if (['.f', '.fechar'].includes(cmd) && t) {
    if (!eStaff(m.member) && t.dono !== m.author.id) return m.reply(`${E.proibido} Sem permissão!`);
    delete db.tickets[m.channel.id]; delete db.userTickets[t.dono]; salvarDB();
    await m.reply(`${E.fechar} Encerrando e deletando...`);
    return setTimeout(async () => {
      try { await m.channel.delete(); } catch { if (m.channel.isThread()) await m.channel.delete(); }
    }, 2000);
  }

  // Notificar usuário
  if (cmd === '.not' && eStaff(m.member) && t) {
  ...
  // Adicionar usuário ao ticket (Somente Staff)
  if (cmd === '.adc' && t) {
    if (!eStaff(m.member)) return m.reply(`${E.proibido} Apenas a equipe pode adicionar membros ao ticket!`);
    ...
    if (m.channel.isThread()) {
      await m.channel.members.add(alvo.id);
    } else {
      await m.channel.permissionOverwrites.edit(alvo.id, { ViewChannel: true, SendMessages: true, AttachFiles: true });
    }
    return m.reply(`${E.ativado} O usuário ${alvo} foi adicionado ao ticket!`);
  }
  ...
  if (cmd === '!setlogs') { const c = m.mentions.channels.first() || m.guild.channels.cache.get(args[0]); if (c) { config.logChannelId = c.id; salvarCfg(); m.reply(`${E.ativado} Canal de logs configurado!`); } }
});
