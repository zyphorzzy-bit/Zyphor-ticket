  ...
  return {
    ...
    components: [
      ...
      new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder().setCustomId('cfg_cat').setLabel('Comando Categoria').setStyle(ButtonStyle.Secondary),
        new ButtonBuilder().setCustomId('cfg_edit').setLabel('Comando Editar').setStyle(ButtonStyle.Secondary),
...
client.on(Events.InteractionCreate, async int => {
  if (int.isChatInputCommand()) {
    ...
    if (int.commandName === 'enviar') {
      const emb = new EmbedBuilder().setColor(0x2b2d31).setTitle(`${E.config} ATENDIMENTO`).setDescription('Selecione uma opção abaixo para abrir seu ticket.');
      let components = [];
  ...
  if (int.isButton() && int.customId.startsWith('cfg_')) {
    ...
    if (int.customId === 'cfg_add') {
      config.opcoes.push({ label: 'Novo Atendimento', emoji: '❓', value: `op_${Date.now().toString().slice(-4)}` });
    }
    ...
    if (int.customId === 'cfg_cat') return int.reply({ content: `${E.alerta} Use no chat:\n\`!setcat ID_CATEGORIA\``, ephemeral: true });
    if (int.customId === 'cfg_edit') return int.reply({ content: `${E.alerta} Use no chat:\n\`!editar 1 Novo Nome\``, ephemeral: true });
...
client.on(Events.MessageCreate, async m => {
  ...
  // Notificar usuário dono do ticket
  if (cmd === '.not') {
  ...
  // Renomear o ticket/canal
  if (cmd === '.nm') {
    if (!eStaff(m.member)) return;
    const t = db.tickets?.[m.channel.id];
    if (!t) return m.reply(`${E.alerta} Este canal não é um ticket.`);
    
    const novoNome = args.join('-').toLowerCase();
    if (!novoNome) return m.reply(`${E.alerta} Digite o novo nome do ticket! Exemplo: \`.nm suporte silva\``);
